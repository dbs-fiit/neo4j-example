...
public class NewMatrix
{
    ...
    public static void main( String[] args ) throws IOException
    {
        ...
        System.out.println( matrix.printNeoFriendsViaCQL() );
    ...
    private Node getNeoNodeViaCQL(){
    	Map<String, Object> params = new HashMap<String, Object>();
    	params.put( "id", matrixNodeId );

	  	Result result = graphDb.execute( "match (n)-[:NEO_NODE]-> (neo) WHERE id(n)={id} return neo", params );
	  	
    	Iterator<Node> n_column = result.columnAs( "neo" );
    ...
    public String printNeoFriendsViaCQL()
    {
        try ( Transaction tx = graphDb.beginTx() )
        {
        	Node neoNode = getNeoNodeViaCQL();
            // START SNIPPET: friends-usage
            int numberOfFriends = 0;
            String output = neoNode.getProperty( "name" ) + "'s friends via CQL:\n";
            
            Map<String, Object> params = new HashMap<String, Object>();
        	params.put( "id", neoNode.getId() );
        	
        	Result result = graphDb.execute( "match path = shortestPath((neo)-[:KNOWS*]-> (friend)) WHERE id(neo)={id} return friend", params );

        	Iterator<Node> n_column = result.columnAs( "friend" );
